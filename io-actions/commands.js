...
module.exports = function(config){
  ...
  return {
    "POST /execute": function(data, callback, socket){
      ...
      if(command.value.indexOf("cd ") === 0) {   var target =
      data.value.replace("cd ","");   if(target.indexOf("~") === 0)     target
      = target.replace("~", getUserHome());   if(target.indexOf(".") === 0 ||
      (target.indexOf("/") !== 0 && target.indexOf(":\\") !== 1))     target =
      path.normalize(path.join(shell.cwd, target));   fs.exists(target,
      function(e){     if(e) { shell.cwd = target;
